...
const int deltaMeasurementUnitsInSec = 10; // seconds
unsigned long deltaSendToGateInMilis = -1; //this is set by initialziing with gate
const int amountOfSensors = 3; //heart rate, skin conductance, skin temperature, muscle tension
//********************************************************************************
...
void performMeasurementsWithSleepInBetween(){
  ...
  for(int i = 0; i < measurementUnitsBeforeSend; i++){
      ...
      //bufferToSend[i] = measurementUnitHeartRateSensor(amountOfHeartRateMeasurementsPerUnit);
      bufferToSend[i + 1 * measurementUnitsBeforeSend] = measurementUnitSkinTemperature(amountOfSkinTempMeasurementsPerUnit);
      bufferToSend[i + 2 * measurementUnitsBeforeSend] = measurementUnitMuscleTension(amountOfMuscleTensionMeasurementsPerUnit);
...
float measurementUnitSkinTemperature(byte size){
  ...
  float sum;
  for(int i = 0; i<size;i++){
    //Serial.println(String(heartRateSensor.readTemperature()));
